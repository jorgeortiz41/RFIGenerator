...
from rfigen.legacy.config_loader import load_config, save_config
...
DEFAULT_CONFIG_NAME = "legacy_base.yaml"


def _default_config_path() -> Optional[Path]:
    """Locate the bundled legacy example config.

    Checked in order: the repository ``configs/`` directory (when running from a
    source checkout) and ``configs/`` under the current working directory.
    Returns None when no candidate exists, leaving the GUI to start unconfigured.
    """
    candidates = [
        Path(__file__).resolve().parents[3] / "configs" / DEFAULT_CONFIG_NAME,
