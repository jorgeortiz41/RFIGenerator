...
DEFAULT_SCAN_POSITIONS: list[tuple[float, float]] = [
    (0.0, 20.0),
    (0.0, 90.0),
    (0.0, 160.0),
    (45.0, 20.0),
...
def generate_level1(
    ...
    seed: int = 42,
    start_utc: str = "",
    scan_positions: list[tuple[float, float]] | None = None,
) -> Path:
    """Write a synthetic MP-3000A Level-1 CSV and return its path.

    Shared entry point for both the command line and the config-driven pipeline,
    ...
    if start_utc.strip():
        start_time = datetime.strptime(start_utc, "%Y-%m-%dT%H:%M:%S").replace(tzinfo=timezone.utc)
    else:
    ...
    write_level1_csv(
        ...
        scan_positions=scan_positions or DEFAULT_SCAN_POSITIONS,
        seed=seed,
    )
    return resolved


def level1_to_wide_frame(csv_path: str | Path, k_band_only: bool = True):
    """Reshape an RTTOV Level-1 CSV into the MP-3000A wide frame.

    :func:`write_level1_csv` emits tidy/long rows (one row per channel per
    observation, record type 51). The rest of the legacy pipeline — the signal
    mixer and exporter — expects the MP-3000A wide layout, one row per
    observation with a ``Ch <freq>`` column per channel. This bridges the two.
    """
    import pandas as pd

    frame = pd.read_csv(csv_path)
    scans = frame[frame["record_type"] == 51].copy()
    if scans.empty:
        raise ValueError(f"no record-type 51 scan rows found in {csv_path}")

    if k_band_only:
        lo, hi = K_BAND_CHANNELS_GHZ[0], K_BAND_CHANNELS_GHZ[-1]
        scans = scans[scans["frequency_ghz"].between(lo, hi)]
        if scans.empty:
            raise ValueError(f"no K-band channels between {lo} and {hi} GHz in {csv_path}")

    scans["channel"] = scans["frequency_ghz"].map(lambda ghz: f"Ch {ghz:7.3f}")
    keys = ["datetime_utc", "azimuth_deg", "elevation_deg"]
    wide = scans.pivot_table(index=keys, columns="channel", values="tb_k", aggfunc="mean")
    wide = wide.reset_index()

    ambient = scans.groupby(keys)["tamb_c"].mean().reset_index()
    wide = wide.merge(ambient, on=keys, how="left")

    channel_columns = sorted(
        (column for column in wide.columns if str(column).startswith("Ch ")),
        key=lambda column: float(str(column).split()[1]),
    )
    wide = wide.rename(
        columns={
            "datetime_utc": "Date/Time",
            "azimuth_deg": "Az(deg)",
            "elevation_deg": "El(deg)",
            "tamb_c": "TkBB(K)",
        }
    )
    wide.insert(0, "Record", range(2, len(wide) + 2))
    ordered = ["Record", "Date/Time", "Az(deg)", "El(deg)", "TkBB(K)", *channel_columns]
    return wide[ordered]


def generate_level1_from_config(config: dict) -> Path:
    """Run the RTTOV generator using the pipeline's config dictionary."""
    radiometry = config.get("radiometry", {})
    run = config.get("run", {})
    export_dir = Path(config.get("export", {}).get("directory", "outputs/"))
    output_path = radiometry.get("rttov_output_path") or export_dir / "rttov_lv1.csv"
    return generate_level1(
        output_path=output_path,
        hours=float(radiometry.get("rttov_hours", 6.0)),
        step_seconds=int(radiometry.get("rttov_step_seconds", 60)),
        seed=int(run.get("seed", 42)),
        start_utc=str(radiometry.get("rttov_start_utc", "")),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Number of scan directions per time step: {len(DEFAULT_SCAN_POSITIONS)}")
    print("Scan directions (Az-El):")
    for azimuth_deg, elevation_deg in DEFAULT_SCAN_POSITIONS:
